...
- (void)viewDidLoad
{
     ...
     */
    if ([allowInlineMediaPlayback boolValue] && [self.config respondsToSelector:@selector(allowsInlineMediaPlayback)]) {
        self.config.allowsInlineMediaPlayback = YES;
    }
    
    if ((mediaPlaybackRequiresUserAction == NO) && [self.config respondsToSelector:@selector(mediaPlaybackRequiresUserAction)]) {
        self.config.mediaTypesRequiringUserActionForPlayback = NO;
    }
...
-(void)shouldAutorotateToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation interface:(void (^)(BOOL))block{
    ...
    [webView evaluateJavaScript:jsCall completionHandler:^(id _Nullable res, NSError * _Nullable error) {
        if ([res length] > 0) {
            block([res boolValue]);
        }else{
            block([self supportsOrientation:interfaceOrientation]);
        }
    }];
}
...
-(void)supportedInterfaceOrientations:(void (^)(NSUInteger))block{
    __block NSUInteger ret = 0;
    
    [webView evaluateJavaScript:[NSString stringWithFormat:
    @"window.shouldRotateToOrientation && window.shouldRotateToOrientation(%ld);"
    , (long)[self mapIosOrientationToJsOrientation:UIInterfaceOrientationPortrait]] completionHandler:^(id _Nullable res, NSError * _Nullable error) {
        if ([res length] > 0 || [self supportsOrientation:UIInterfaceOrientationPortrait]) {
    ...
    [webView evaluateJavaScript:[NSString stringWithFormat:
    @"window.shouldRotateToOrientation && window.shouldRotateToOrientation(%ld);"
    , (long)[self mapIosOrientationToJsOrientation:UIInterfaceOrientationPortraitUpsideDown]] completionHandler:^(id _Nullable res, NSError * _Nullable error) {
        if ([res length] > 0 || [self supportsOrientation:UIInterfaceOrientationPortraitUpsideDown]) {
    ...
    [webView evaluateJavaScript:[NSString stringWithFormat:
    @"window.shouldRotateToOrientation && window.shouldRotateToOrientation(%ld);"
    , (long)[self mapIosOrientationToJsOrientation:UIInterfaceOrientationLandscapeRight]] completionHandler:^(id _Nullable res, NSError * _Nullable error) {
        if ([res length] > 0 || [self supportsOrientation:UIInterfaceOrientationLandscapeRight]) {
    ...
    [webView evaluateJavaScript:[NSString stringWithFormat:
    @"window.shouldRotateToOrientation && window.shouldRotateToOrientation(%ld);"
    , (long)[self mapIosOrientationToJsOrientation:UIInterfaceOrientationLandscapeLeft]] completionHandler:^(id _Nullable res, NSError * _Nullable error) {
        if ([res length] > 0 || [self supportsOrientation:UIInterfaceOrientationLandscapeLeft]) {
...
{
    return [[WKWebView alloc] initWithFrame:bounds configuration:self.config];
}
...
{
    if (_userAgent == nil) {
        [CDVUserAgentUtil originalUserAgent:^(NSString *UserAgent) {
            _userAgent = [NSString stringWithFormat:@"%@ (%lld)", UserAgent, (long long)self];
        }];
    }
    return _userAgent;
}
