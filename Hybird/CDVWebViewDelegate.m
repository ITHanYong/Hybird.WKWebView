...
{
    _curLoadToken += 1;
    [webView evaluateJavaScript:[NSString stringWithFormat:@"window.__cordovaLoadToken=%ld", (long)_curLoadToken] completionHandler:nil];
}

//- (NSString*)evalForCurrentURL:(WKWebView*)webView
//{
//    [webView evaluateJavaScript:@"location.href" completionHandler:^(id _Nullable url, NSError * _Nullable error) {
//
//    }];
...
- (void)pollForPageLoadStart:(WKWebView*)webView
{
    ...
    [webView evaluateJavaScript:@"window.__cordovaLoadToken" completionHandler:^(id _Nullable loadToken, NSError * _Nullable error) {
        BOOL jsLoadToken = [[NSString stringWithFormat:@"%ld", (long)_curLoadToken] isEqualToString:loadToken];
...
- (void)pollForPageLoadFinish:(WKWebView*)webView
{
    ...
    [webView evaluateJavaScript:@"document.readyState" completionHandler:^(id _Nullable state, NSError * _Nullable error) {
        
        BOOL ready = [state isEqualToString:@"loaded"] || [state isEqualToString:@"complete"];
